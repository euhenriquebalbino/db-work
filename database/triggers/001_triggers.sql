...
BEGIN
  ...
  IF EXISTS (SELECT 1
  FROM inserted
  WHERE nota1 NOT BETWEEN 0 AND 10 OR nota2 NOT BETWEEN 0 AND 10 OR nota3 NOT BETWEEN 0 AND 10)
  BEGIN
...
BEGIN
  ...
  INSERT INTO dbo.audit_logs
    (tabela, operacao, registro_id, detalhes)
  SELECT 'alunos',
    CASE WHEN EXISTS (SELECT 1
      FROM inserted) AND EXISTS (SELECT 1
      FROM deleted) THEN 'UPDATE'
              WHEN EXISTS (SELECT 1
    FROM inserted) THEN 'INSERT' ELSE 'DELETE' END,
    COALESCE(i.id, d.id),
