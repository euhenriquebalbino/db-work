...
export class NotasController {
  ...
  @Post() create(@Body() dto: CreateNotaDto) {
  ...
  @Get("aluno/:alunoId") byAluno(
    @Param("alunoId", ParseIntPipe) alunoId: number,
  ) {
    return this.service.byAluno(alunoId);
  }
  @Get("turma/:turmaId") byTurma(
    @Param("turmaId", ParseIntPipe) turmaId: number,
  ) {
    return this.service.byTurma(turmaId);
  }
