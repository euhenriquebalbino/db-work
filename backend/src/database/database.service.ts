...
export type ProcedureParamValue = string | number | boolean | Date | null | undefined;
...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  async executeProcedure<T>(procedure: string, params: object = {}): Promise<T[]> {
    if (!this.pool) {
      throw new Error('Pool SQL Server nao inicializado');
    }

    const request = this.pool.request();
    Object.entries(params ?? {}).forEach(([key, value]) => request.input(key, value as ProcedureParamValue));
    try {
      const result = await request.execute<T>(procedure);
      return result.recordset ?? [];
    } catch (error) {
      const sqlError = error as { number?: number; message?: string; originalError?: { info?: { message?: string } } };
      const message = sqlError.originalError?.info?.message ?? sqlError.message ?? 'Erro ao executar procedure.';
      ...
      if (sqlError.number === 547) {
        throw new ConflictException('Nao e possivel excluir ou alterar este registro porque existem dados vinculados a ele.');
      }
